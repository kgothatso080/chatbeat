...
// AUTO DELETE AFTER 7 DAYS = 604800000 ms
...
setInterval(() => { deleteOld('posts'); deleteOld('globalChat'); }, 3600000); // check every hour
...
function loadChat() {
  onChildAdded(query(ref(db, 'globalChat'), orderByChild('timestamp')), async (snap) => {
    ...
    if(msg.audio) {
      msgDiv.innerHTML = `<b style="color:${user.color}">${user.name}:</b> <audio controls src="${msg.audio}" style="width:200px;"></audio>`;
    } else {
...
// 7. SWITCH BETWEEN FEED AND CHAT
document.getElementById('chat-icon-btn').onclick = () => {
  document.getElementById('feed-section').style.display = 'none';
  document.getElementById('chat-section').style.display = 'block';
}

// Click logo to go back to feed
document.querySelector('.topbar h2').onclick = () => {
